...
def calculate(req: CalculateRequest):
    ...
    # Validating solved domain nodes only
    ...
    if internal_fdm.size > 0:
        ...
        validation_index = 100.0

    df = pd.DataFrame([{"grid_size": req.m, "iterations": iters, "time_taken": solve_time}])
    df.to_csv("solver_runs.csv", mode='a', header=not os.path.exists("solver_runs.csv"), index=False)
    ...
    return {
        ...
        "time": solve_time, 
        "iters": iters, 
        "validation_score": float(validation_index)
    }
