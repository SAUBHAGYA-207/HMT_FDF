...
def red_black_sor_solver(Tu, Td, Tl, Tr, n, tol=1e-4, max_iter=5000):
    ...
    for iter_count in range(max_iter):
        ...
        for mask in [red_mask, black_mask]:
            north = torch.zeros_like(T); north[1:, :] = T[:-1, :]; north[0, :] = Td
            south = torch.zeros_like(T); south[:-1, :] = T[1:, :]; south[-1, :] = Tu
...
def get_analytical_matrix(m, Tu, Td, Tl, Tr):
    ...
    y_vals = np.linspace(0, m, m + 1) # Y starts at 0 (bottom)
    ...
    for n in range(1, 2 * terms, 2):
        ...
        # Td at Y=k (Top), Tu at Y=0 (Bottom)
        btm_top = (Td * np.sinh(n_pi_k * Y) + Tu * np.sinh(n_pi_k * (k - Y))) * np.sin(n_pi_k * X)
        lft_rgt = (Tl * np.sinh(n_pi_k * (k - X)) + Tr * np.sinh(n_pi_k * X)) * np.sin(n_pi_k * Y)
        grid += coeff * (btm_top + lft_rgt)
    
    grid = np.flipud(grid) # Flip to match matrix row indexing (0 at top)
    grid[0, :] = Td; grid[-1, :] = Tu
    grid[:, 0] = Tl; grid[:, -1] = Tr
    return grid
...
def calculate(req: CalculateRequest):
    ...
    fdm_full[0, :] = req.Td; fdm_full[-1, :] = req.Tu
...
def get_regression():
    ...
    if len(df) < 3: return {"error": "Need 3+ points"}
